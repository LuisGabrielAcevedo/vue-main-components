<template>
  <v-flex v-if="visibleValue">
    ...
    <v-select
      ...
      :items="options"
      v-model="value"
      :item-value="field.options.associationValue"
      :item-text="field.options.associationText"
      :name="field.key"
    ...
    >
      <template slot="selection" slot-scope="{ item, index }">
        <span v-if="index === 0">{{ item[field.options.associationText] ? item[field.options.associationText] : item }}</span>
        <span
          v-if="index === 1 && value"
